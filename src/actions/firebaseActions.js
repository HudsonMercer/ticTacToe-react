...
export function fireSendData(
  firebase,
  destination = '',
  data = { debug: 'ERROR BAD DATA SENT' },
  thenCallback,
) {
  return dispatch => {
  ...
  return dispatch => {
    firebase
      ...
      .then(authData => {
        ...
        firebase
          .storage()
          .ref(`/userProfiles/${authData.uid}/avatar/image`)
          .getDownloadURL()
          .then(url => {
    ...
    firebase
      ...
      .then(e => {
        dispatch(
          setSplashErrorData(
            'New user created',
            'Ready to login now with the E-mail and password you used.',
            'New Account Created',
          ),
        );
        dispatch(toggleSplashError(true));
...
export function fireSaveColorScheme(colorScheme, firebase) {
  return dispatch => {
    firebase
      .update(
        `userProfiles/${store.getState().userState.uid}/colorScheme`,
        colorScheme,
      )
      .then(dispatch(fireSaveColorSchemeComplete()));
  };
}
...
export function fireGetColorScheme(firebase) {
  return dispatch => {
    firebase
      .database()
      .ref(`userProfiles/${store.getState().userState.uid}/colorScheme`)
      .once('value', data => {
        if (data.val() !== null) {
          dispatch(
            setColorScheme(data.val().red, data.val().green, data.val().blue),
          );
        }
      });
  };
...
export function fireUserLeaveGame(firebase, userUid, gameUid) {
  return dispatch => {
    ...
    dispatch(
      fireSendData(firebase, `/lobby/games/${gameUid}/`, { leaverId: userUid }),
    );
  };
}
