...
export default class GameBoard extends Component {
  getUserTurn = () => {
    try {
      switch (true) {
        case this.props.gameState.playerTurn === 'host' &&
          this.props.userState.isHost:
          return 'Your turn';
        case this.props.gameState.playerTurn === 'client' &&
          !this.props.userState.isHost:
          return 'Your turn';
        case this.props.gameState.playerTurn === 'host' &&
          !this.props.userState.isHost:
  ...
  victoryBannerHandeler = () => {
    try {
      if (this.props.victory === true) {
        ...
        const GameBoardWinBannerShadow = (
          <div style={{ height: '0px' }} className="gameBoardWinBannerShadow" />
        );
        return GameBoardWinBannerShadow;
      }
    } catch (err) {
      ...
      case boardState[0] !== 'e' &&
        ...
        this.props.fireSendData(
          ...
          `lobby/games/${this.props.userState.gameUid}`,
          {
            ...
            winner: boardState[0],
            position: 'rowTop',
            winDialogIsOpen: true,
          },
        );
        this.props.fireAddScore(
          this.props.firebase,
          this.props.userState.gameUid,
          boardState[0],
        );
    ...
    try {
      ...
      return (
        <Card className="gameBoardCard">
          ...
          <Toolbar>
            <ToolbarRow>
              <ToolbarSection>{playerTurn}</ToolbarSection>
              <ToolbarSection>
                <Headline className="no-mobile">
                  {this.getTitleStatus()}
                </Headline>
              </ToolbarSection>
              <ToolbarSection>
          ...
          <CardActions>
            <Button compact raised onClick={this.leaveGameHandler}>
              Leave Game
            </Button>
          </CardActions>
        </Card>
